...
namespace cgue
{
	...
	public:
		Geometry();
		virtual ~Geometry();
		void init(const std::string& displayFile, const std::string& collisionFile, glm::mat4& matrix, Shader* _shader, glm::mat4x4 initTrans);
	...
	private:
		...
		void searchNodesRecursive(aiNode* node, const aiScene* scene, glm::mat4x4 initTrans, float* posi, float* norm, int* ind, float* u, Mesh** mesh, Shader* shader);
