...
void Geometry::searchNodesRecursive(aiNode* node, const aiScene* scene, mat4x4 initTrans, float* positionsPointer, float* normalsPointer, int* indicesPointer, float* uvsPointer, Mesh** meshPointer, Shader* shader)
{
	...
	for (int i = 0; i < node->mNumMeshes; i++)
	{
		...
		meshPointer[meshIndex++] = new Mesh(name.data, mesh, material, initTrans, transform, shader);
	...
	{
		searchNodesRecursive(node->mChildren[i], scene, initTrans, positionsPointer, normalsPointer, indicesPointer, uvsPointer, meshPointer, shader);
	}
}
...
void Geometry::init(const std::string& displayFile, const std::string& collisionFile, mat4& matrix, Shader* _shader, mat4x4 initTrans) {
	...
	const aiScene* scene = displayImporter.ReadFile(displayFile, aiProcessPreset_TargetRealtime_Quality);
	if (!scene)
	{
	...
	Assimp::Importer collisionImporter;
	const aiScene* collisionScene = collisionImporter.ReadFile(collisionFile, aiProcessPreset_TargetRealtime_Quality);
	if (!collisionScene)
	{
		std::cout << "Couldn't load model ";
		glfwTerminate();
	...
	int collisionNumMesh = collisionScene->mNumMeshes;
	if (collisionNumMesh != 1) {
		std::cout << "Collision Mesh may only contain one mesh ";
		glfwTerminate();
		system("PAUSE");
		exit(EXIT_FAILURE);
	}
	collisionSize = 0;
	aiMesh* collisionMesh = collisionScene->mMeshes[0];
	for (int j = 0; j < collisionMesh->mNumFaces; j++)
	{
		const aiFace& face = collisionMesh->mFaces[j];
		for (int k = 0; k < face.mNumIndices; k++)
		{
			collisionSize++;
		}
	}
	collisionPositions = new float[collisionSize * 3];
	collisionIndices = new int[collisionSize];
	...
	for (int j = 0; j < collisionMesh->mNumFaces; j++)
	{
		...
		for (int k = 0; k < face.mNumIndices; k++)
		{
			aiVector3D aiPos = collisionMesh->mVertices[face.mIndices[k]];
			vec4 pos = vec4(aiPos.x, aiPos.y, aiPos.z, 1);

			collisionPositions[iVertices * 3 + 0] = pos.x;
			collisionPositions[iVertices * 3 + 1] = pos.y;
			collisionPositions[iVertices * 3 + 2] = pos.z;

			collisionIndices[iVertices] = iVertices;
	...
	char* nameMesh = "";
	test = new Mesh(nameMesh, collisionMesh, material, mat4x4(1.0f), mat4x4(1.0f), shader);

	iVertices = 0;
	meshIndex = 0;
	Geometry::searchNodesRecursive(rootNode, scene, initTrans, positions, normals, indices, uvs, mesh, shader);
}
...
Geometry::~Geometry()
{
	...
	delete collisionPositions; collisionPositions = nullptr;
	delete collisionIndices; collisionIndices = nullptr;
